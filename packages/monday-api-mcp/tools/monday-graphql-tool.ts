...
// export const registerMondayCreateItemTool = (mcp: McpServer) => {
//   mcp.tool(
//     'create monday item',
//     'Create a new item in a Monday.com board using natural language',
//     {
//       inputSchema: z.object({
//         request: z
//           .string()
//           .describe(
//             'Natural language request to create an item, e.g. "Create a task called Fix API bug on board 123456"',
//           ),
//       }),
//     },
//     async ({ inputSchema }) => {
//       const graphqlHandler = GraphQLHandler.getInstance();
//       const request = inputSchema.request.toLowerCase();
...
//         if (!mutationType) {
//           throw new Error("GraphQL schema doesn't have a mutation type");
//         }

//         // Get all available mutation fields
//         const mutationFields = mutationType.getFields();

//         // Look for item creation mutations
//         const createItemFieldNames = Object.keys(mutationFields).filter(
//           (name) =>
//             name.toLowerCase().includes('create') &&
//             (name.toLowerCase().includes('item') || name.toLowerCase().includes('task')),
//         );

//         if (createItemFieldNames.length === 0) {
//           throw new Error("Couldn't find a mutation for creating items");
//         }

//         // Use the first matching mutation (usually create_item)
//         const createItemField = mutationFields[createItemFieldNames[0]];

//         // Extract board ID from the request
//         const boardIdMatch = request.match(/\bboard\s*:?\s*(\d+)\b|\bon\s+board\s+(\d+)\b/);
//         const boardId = boardIdMatch ? boardIdMatch[1] || boardIdMatch[2] : null;
...
//               {
//                 type: 'text' as const,
//                 text: 'Please specify a board ID in your request. For example: "Create a task called Fix API bug on board 123456"',
//               },
//             ],
//             _meta: {},
//           };
...
//         // Extract item name from the request
//         const itemNameMatch = request.match(/\bcalled\s+([^"]+)|\bnamed\s+([^"]+)|\bitem\s+([^"]+)|\btask\s+([^"]+)/);
//         let itemName = itemNameMatch
//           ? itemNameMatch[1] || itemNameMatch[2] || itemNameMatch[3] || itemNameMatch[4]
//           : null;

//         // If no match found with the patterns above, try looking for quoted text
//         if (!itemName) {
//           const quotedMatch = request.match(/"([^"]+)"/);
//           itemName = quotedMatch ? quotedMatch[1] : null;
//         }
...
//         // Find the expected argument names from the schema
//         const requiredArgNames = createItemField.args.map((arg: any) => arg.name);

//         // Map our extracted values to the schema-defined argument names
//         const variables: Record<string, any> = {};

//         // Look for board_id argument
//         if (requiredArgNames.includes('board_id')) {
//           variables.board_id = boardId;
//         }

//         // Look for item_name argument
//         if (requiredArgNames.includes('item_name')) {
//           variables.item_name = itemName.trim();
//         } else if (requiredArgNames.includes('name')) {
//           variables.name = itemName.trim();
//         }

//         // Build the create item mutation dynamically
//         // Get the return fields dynamically based on the type returned by the mutation
//         const returnFields = getCommonFieldsForType('item').filter(
//           (field) => !field.includes(' ') && !field.includes('{'),
//         ); // Filter out nested fields for simplicity

//         const mutation = `
//           mutation createItem(${createItemField.args
//             .map((arg: any) => `$${arg.name}: ${getGraphQLTypeString(arg.type)}`)
//             .join(', ')}) {
//             ${createItemFieldNames[0]}(${createItemField.args
...
//         const { content, _meta } = await graphqlHandler.execute<GraphQLResponse>(mutation, variables, (data) => {
//           if (!data) return 'Failed to create item';
...
//         return {
//           content: [
//             { type: 'text' as const, text: 'Created a new item in Monday.com:' },
//             { type: 'text' as const, text: 'Generated Mutation:' },
//             { type: 'text' as const, text: mutation },
//             { type: 'text' as const, text: '\nVariables:' },
//             { type: 'text' as const, text: JSON.stringify(variables, null, 2) },
//             { type: 'text' as const, text: '\nResponse:' },
//             ...content,
...
//             {
//               type: 'text' as const,
//               text: `Failed to create Monday.com item: ${error instanceof Error ? error.message : 'Unknown error'}`,
//             },
//           ],
//           _meta: {},
//         };
...
// export const registerMondaySimpleCreateItemTool = (mcp: McpServer) => {
//   mcp.tool(
//     'mcp_create_monday_item',
...
//         // Map our input values to the schema-defined argument names
//         const variables: Record<string, any> = {};

//         // Look for board_id argument
//         if (requiredArgNames.includes('board_id')) {
//           variables.board_id = inputSchema.boardId;
//         }

//         // Look for item_name argument
//         if (requiredArgNames.includes('item_name')) {
//           variables.item_name = inputSchema.itemName;
//         } else if (requiredArgNames.includes('name')) {
//           variables.name = inputSchema.itemName;
//         }
...
//         return {
//           content: [
//             { type: 'text' as const, text: `Created item "${inputSchema.itemName}" in board ${inputSchema.boardId}:` },
//             { type: 'text' as const, text: 'Generated Mutation:' },
//             { type: 'text' as const, text: mutation },
//             { type: 'text' as const, text: '\nVariables:' },
//             { type: 'text' as const, text: JSON.stringify(variables, null, 2) },
